package com.ld.gg.paging;

public class Paging {
	private int maxNum; 		// 전체 글의 개수
	private int pageNum; 		// 현재 페이지 번호
	private int listCount; //100		// 페이지당 나타낼 글의 갯수
	private int pageCount; //2		// 페이지그룹당 페이지 갯수  
	private String url; 	// 게시판의 종류(url) //
	
	public Paging(int maxNum, Integer pageNum, int listCount, int pageCount, String url) {
		this.maxNum = maxNum;
	...
	public String makeHtmlPaging() {
		...
		return makeHtml(currentGroup, totalPage, url);
	}

	private String makeHtml(int currentGroup, int totalPage, String url) {
		...
		if (start != 1) {
			sb.append("<a href='"+url+"?pageNum=" + (start -1) + "'>");
			sb.append("[이전]");
			sb.append("</a>");
		}
		...
		for (int i = start; i <= end; i++) {
			if (pageNum != i) { //현재 페이지가 아닌 경우 링크처리
				sb.append("<a href='"+url+"?pageNum=" + i + "'>");
				sb.append(" [ ");
				sb.append(i);
				sb.append(" ] ");
				sb.append("</a>");
		...
		if (end != totalPage) {
			sb.append("<a href='"+url+"?pageNum=" + (end + 1) + "'>");
			sb.append("[다음]");
			sb.append("</a>");
		}
		return sb.toString();
